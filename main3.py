...
class AttentionLayer(Layer):
    ...
    def build(self, input_shape):
        ...
        self.w = self.add_weight(name='gamma', 
                              shape=[L_size,1],
                              initializer='uniform',
                              trainable=True)
        self.V = self.add_weight(name='gamma', 
                      shape=[input_shape[2],L_size],
                      initializer='uniform',
                      trainable=True)

        self.U = self.add_weight(name='gamma', 
                      shape=[input_shape[2],L_size],
                      initializer='uniform',
                      trainable=True)
        ...
        self.bias_w = self.add_weight(shape=(filters_f_g,),
                              initializer='zeros',
                              name='bias')
        self.bias_V = self.add_weight(shape=(filters_f_g,),
                                      initializer='zeros',
                                      name='bias')
        self.bias_U = self.add_weight(shape=(filters_h,),
                                      initializer='zeros',
                                      name='bias')
      """
...
class FCHeadNet:
  ...
  def build(baseModel, classes, D):
    ...
    headModel = AttentionLayer()(headModel)
